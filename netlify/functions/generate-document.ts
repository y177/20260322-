...
{
  "title": "이메일 제목",
  "result": "완성된 이메일 전문 (수신자, 발신자, 제목, 본문, 마무리 인사 포함)"
}`,
...
{
  "title": "보고서 제목",
  "result": "완성된 보고서 (개요, 주요 내용, 현황, 결론 및 향후 계획 포함)"
}`,
...
{
  "title": "회의록 제목",
  "result": "완성된 회의록 (일시, 참석자(미기재), 안건, 논의 내용, 결정 사항, 향후 액션 아이템 포함)"
}`,
...
{
  "title": "기획서 제목",
  "result": "완성된 기획서 (배경 및 목적, 현황 분석, 제안 내용, 기대 효과, 실행 계획 포함)"
}`,
...
{
  "title": "업무 일지",
  "result": "완성된 업무 일지 (날짜(오늘), 주요 업무, 완료 사항, 미완료 사항 및 사유, 특이 사항, 내일 계획 포함)"
}`,
...
{
  "title": "요약 제목",
  "result": "완성된 요약 (핵심 요약(3-5줄), 주요 포인트(불릿 포인트), 결론 또는 액션 아이템 포함)"
}`,
};
...
export const handler: Handler = async (event: HandlerEvent, _context: HandlerContext) => {
  ...
  if (!body.taskType || !body.content || body.content.trim().length === 0) {
    return {
      statusCode: 400,
      headers: corsHeaders,
      body: JSON.stringify({ error: '업무 유형과 내용을 입력해주세요.' }),
    };
  }
  ...
  if (!systemPrompt) {
    return {
      statusCode: 400,
      headers: corsHeaders,
      body: JSON.stringify({ error: '지원하지 않는 업무 유형입니다.' }),
    };
  }
  ...
  try {
    ...
    if (message.includes('429') || message.includes('Too Many Requests') || message.includes('quota')) {
      return {
        statusCode: 429,
        headers: corsHeaders,
        body: JSON.stringify({ error: 'API 사용량 한도를 초과했습니다. 잠시 후 다시 시도해주세요.' }),
      };
    }
    ...
    return {
      statusCode: 500,
      headers: corsHeaders,
      body: JSON.stringify({ error: `문서 생성 중 오류가 발생했습니다: ${message}` }),
    };
  }
};
